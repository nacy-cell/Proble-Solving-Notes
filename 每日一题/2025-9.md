...
哈希表映射，找到所有等于最大频率的元素个数，返回`个数*频率`即可
...
handle函数把string处理成int数组，然后补齐长度不相等的部分，依次比较即可
...
```cpp
void handle(vector<int>& nums,string& s) {
        int current=0;
        while (true) {
            int point=s.find(".",current);
            if (point==string::npos) {
                nums.push_back(stoi(s.substr(current)));
               break;
            }
            nums.push_back(stoi(s.substr(current,point-current)));
            current=point+1;
        }
    }
    ...
    int compareVersion(string version1, string version2) {
        ...
        handle(nums_1,version1);
        handle(nums_2,version2);
        while (nums_1.size()<nums_2.size()) {nums_1.push_back(0);}
        while (nums_2.size()<nums_1.size()) {nums_2.push_back(0);}
        for (int i=0;i<nums_1.size();i++) {
            if (nums_1[1]!=nums_2[1]) {
                return nums_1[1]>nums_2[1]?1:-1;
            }
        }
        return 0;
    }
...
思路很简单，模拟，一直除，直到整除或者出现相同的被除数就说明是无限循环小数

需要注意数据范围；

1. numerator,denominator负数范围可以到-2^31^,如果求正会会让int溢出，所以最好转化为long long 处理
2. numerator可能为0,如果denominator为负数，可能出现-0的结果，最好把0单独处理
...
经典动态规划板题，状态转移方程`dp[i][j]=min(dp[i-1][j],dp[i-1][j-1])+triangle[i][j]`
...
```cpp
    int minimumTotal(vector<vector<int>>& triangle) {
        ...
        vector sum(height,vector(height,0));

        sum[0][0]=triangle[0][0];
        for (int i=1;i<height;++i) {
            for (int j=0;j<=i;j++) {
                if (j==i)sum[i][j]=triangle[i][j]+sum[i-1][j-1];
                else if (j==0)sum[i][j]=triangle[i][j]+sum[i-1][j];
                else sum[i][j]=min(sum[i-1][j],sum[i-1][j-1])+triangle[i][j];
            }
        }
        int ans=0x3fffffff;
        for (int i=0;i<height;++i)
            ans=min(ans,sum[height-1][i]);
        return ans;
    }
```
...
三角形组成条件：两短边之和小于第三边
因此遍历前两边，二分找出能小于两边和的最大的第三边（即大于等于和的前一个数），累加即可。时间复杂度O(n^2^log(n)).

双指针优化：小于两条较小边之和的边依旧小于两条更大的边的和，时间复杂度O(n^2^)
`ans+=max(k-j-1,0)`是因为k的起点是i+2,可能会小于j,而如果`num[i]==0`,就会导致k与j重合（`num[j]+num[i]==num[k]`使循环终止）,就会让`k-j-1`变成-1,显然不合题意，因此要保证最小为0.而二分不需要max是因为搜索起点是`j+1`,确保了结果始终为自然数。
    ...
    int triangleNumber(vector<int>& nums) {
        ...
        for (int i=0;i<n-2;i++) {
            for (int j=i+1;j<n-1;j++) {
                int c=nums[i]+nums[j];
                int count=lower_bound(nums.begin()+j+1,nums.end(),c)-nums.begin()-j-1;
                ans+=count;
            }
        }
        return ans;
...
```cpp
    double largestTriangleArea(vector<vector<int>>& points) {
        ...
        for (auto&&a:points)
            for(auto &&b:points)
                for (auto&&c:points) {
                    int x1=a[0],x2=b[0]-x1,x3=c[0]-x1;
                    int y1=a[1],y2=b[1]-y1,y3=c[1]-y1;
